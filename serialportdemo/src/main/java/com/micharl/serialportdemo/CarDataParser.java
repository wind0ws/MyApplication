...
import static com.micharl.serialportdemo.SerialPortBase.convertIntegers;
...
public class CarDataParser {
    ...
    public int[] getOneFrameWithoutSpecialChar(int[] oneFrame){
        if(oneFrame==null||oneFrame.length<11){
            Log.d(TAG,"获取的帧为空或者帧大小不够11个");
            return null;//不够完整一个帧
        }
...
//        }else{
    ...
    public int getFunctionCode(int[] frameFromQueue,int frameHead,int destinationCode, int unitCode){
        if(frameFromQueue[0]==frameHead&&frameFromQueue[1]==destinationCode){
    ...
    public int handleSpecialHex(int special)  {
        switch (special){
            ...
            case 0x04:
                return 0xff;
            default:
                throw new IllegalArgumentException("handleSpecialHex字符码不在定义中");
        }
    }
